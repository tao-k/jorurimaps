...
module Gis::NameValue
  ...
  def self.get_cache(type, section, key, criteria='')
    c_key = "Gis::NameValue.get_" + type.to_s + "_" + section.to_s + "_" + key.to_s + "_" + criteria.to_s
    begin
  ...
  def self.get(type, section, key, criteria='')
    case type
    when 'yaml'
      wrk_hash_raw = Gis.load_yaml_files(section)
  ...
  def self.hash_getter(hash, key)
    ...
    if split_key.length == 1
      last_key = Gis.to_int(split_key[0]) rescue split_key[0]
