...
#      list.each {|k, v| radio_1line.call}
    ...
    return !opt_return_array.blank? ? html_a : "#{options[:prefix]}#{Gis.join(html_a, options[:delim])}#{options[:suffix]}".html_safe
  end
  ...
  def date_picker3(f, name, value=nil, options={})
    ...
    options[:id] = Gis.name_to_id(tag_name)
    options[:format] = :db
    options[:image] = '/_common/themes/gis/files/icon/ic_act_calendar.gif'
  ...
  def date_picker5(f, name, value=nil, _options={})
    ...
    ret += %Q(<button type="button" id="#{tag_id}_bt" onclick="showCalendar('#{tag_id}_bt','#{tag_id}')" class="show_cal_bt"></button>)
    if !options[:time].nil?
      #ret += f.select(:category_id, Gis.yaml_to_array_for_select('gw_todos_category'), :selected=>@item.category_id)
      ret += %Q( #{select_tag("item[schedule_props][prop_type_id]",Gis.options_for_select((0..23).to_a.collect{|x| [x,x]}, :include_blank=>1))} 時) +
        %Q( #{select_tag("item[schedule_props][prop_type_id]",Gis.options_for_select([0,15,30,45].collect{|x| [x,x]}, :include_blank=>1))} 分)
    end
    ret = %Q(<span class="field_with_errors">#{ret}</span>) if !err_flg.nil?
  ...
  def date_picker6(f, name, _options={})
    ...
    value = Gis.to_time(value) if value.is_a?(String)
    ...
    options_calendar_date_select = {
      ...
      :image=>'/_common/themes/gis/files/icon/ic_act_calendar.gif',
    }
    datetime_part = lambda{|idx, _select_options_a, selected|
      ...
      select_tag(_name, mock_for_select(select_options_a, :value_as_label=>1, :to_s=>1, :selected=>selected),
          :id=>Gis.idize(_name), :onchange=> "update_#{tag_id}();") +
        captions_ind[idx - 1]
    }
...
EOL
    ...
    ret += !%w(date datetime).index(mode).nil? ?
      ...
      hidden_field_tag(tag_name, Gis.time_str(value))
    ret = %Q(<span class="field_with_errors">#{ret}</span>) if !err_flg.nil?
    ret += <<-EOL
<script type="text/javascript">
//<![CDATA[
var update_#{tag_id} = function() {
  mode = $('#{Gis.idize(init_tag_name)}').value;
  if (mode == 'datetime' || mode == 'date')
    ymd = $('#{Gis.idize("#{tag_name}_1i")}').value + '-' + $('#{Dms.idize("#{tag_name}_2i")}').value + '-' + $('#{Dms.idize("#{tag_name}_3i")}').value;
EOL
    if tag_id == 'item_st_at'
      ret += <<-EOL
  if (mode == 'datetime' || mode == 'time') {
    hn = $('#{Gis.idize("#{tag_name}_4i")}').value + ':' + $('#{Dms.idize("#{tag_name}_5i")}').value;
    var hne = $('#{Gis.idize("#{tag_name}_4i")}').value;
    hne-=0;
    hne = hne + 1;
    hne+"";
    var hned = hne + ':' + $('#{Gis.idize("#{tag_name}_5i")}').value;
  }
  var sta;
  var stb;
  ...
  def field_with_errors(object, method, &block)
    if block_given?
      if object.errors[method].empty?
        ...
        concat ActionView::Base.field_error_proc.call(capture(&block), object.errors[method])
      end
    end
  end

  def simple_error_messages_for(object)
    if object.errors.size > 0
      msg = object.errors.full_messages.map{|msg| content_tag(:li, msg)}.join.html_safe
      return content_tag(:ul, msg, :class => 'simpleErrorExplanation')
    end
  end

  def unique_error_messages_for(objects)
    return '' if objects.first.blank?

    item = objects.first.class.new
    objects.each do |object|
      object.errors.full_messages.each do |msg|
        if item.errors.full_messages.find{|msg2| msg == msg2}.blank?
