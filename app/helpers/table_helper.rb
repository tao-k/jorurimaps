...
#     ret = Gis.date_common(item.ed_at)
...
#   table_to_index2 @items, :class => 'index print'
  def table_to_form(f, item, _options = {})
    ...
    cr = "\n"
    require 'yaml'
    trans_hash_raw = Gis.load_yaml_files_cache
    ...
    cols.each do |col|
      ...
      field_str, value_str = if textarea_fields.index(col).nil?
        ...
        when 'r'
          [split_trans[1], f.select(col, Gis.yaml_to_array_for_select(split_trans[2]), :selected=>@item.send(col))]
        else
      ...
      field_str += Gis.required if !required_cols.index(col).nil?
...
private
  ...
  def class_str(s, options = {})
    c_s = nil
    _class_name = ((!options[:force_td_class].nil? || !options[:force_th_class].nil?) && Gis.trim(s) != '') ? "#{s}" : ''
  ...
  def get_fv(trans, item, col, trans_hash_raw, options={})
    options[:noh_cols] = Gis.as_array options[:noh_cols]
    split_trans = trans.split(':')
    ...
    if item.methods.grep(col).blank? && item.attributes.keys.grep(col).blank?
      ...
      when /f\{(.+)\}/
        [split_trans[1], Gis.int_format($1, item.send(col))]
      when 'd'
        d = item.send(col)
