...
module GwHelper
  ...
  def mock_for_select(str, _options={})
    ...
    return options[:to_s].blank? ? ret : Gis.options_for_select(ret, options[:selected])
  end
  ...
  def show_group_link(id, options={})
    opt_user_types = Gis.yaml_to_array_for_select('gw_users_class_types')
...
#        opt_user_types += [ ["-----------------" , "-" ] ]
    ...
    opt_user_types = Gis.options_for_select(opt_user_types, options[:selected], :include_blank=>include_blank, :to_s=>1, :title=>:n1)
    opt = options.dup
    opt.delete :selected
  ...
  def show_all_groups_hidden_popups(d, options={})
    ...
    System::Group.find(:all, :conditions=>'level_no=2').each do |x|
      ...
      System::Group.find(:all, :conditions=>"parent_id=#{x.id}").each do |y|
        ret += %Q(\n<li class="yuimenuitem"><a class="yuimenuitemlabel" href="/gw/schedules?gid=#{y.id}&amp;s_date=#{Gis.date8_str(d)}">#{y.name}</a></li>)
      end
      ret += %Q(</ul></div></div></li>)
    end
    ret += '</ul></div></div>'
...
//]]>
  ...
  def get_form_title_div(_params)
    ...
    %Q(<div class="#{Gis.join(class_a, ' ')}">#{get_form_title _params}</div>)
  end
...
EOL
  ...
  def memos_tabbox_struct(send_cls, _qsa)
    qsa = _qsa.nil? ? [] : _qsa
    qs_without_send_cls = Gis.qsa_to_qs(qsa.select{|x| x[0] != 's_send_cls'},{:no_entity=>true})
  ...
  def out_form_ind(key, options={})
    ...
    trans_a = Gis.yaml_to_array_for_select("gw_#{key}_settings_ind")
    ...
    content_for :form do
      form_for :item, :url => "/gw/schedules/edit_ind_#{key}", :html => {:method=>:put, :multipart => true} do |f|
        concat nz(Gis.error_div(item['errors']))
        ...
        trans_a.each do |_trans|
          ...
          field_str, value_str = if textarea_fields.index(col).nil?
            case split_trans[0]
            when 'r'
              [split_trans[1], select_tag(tag_name, Gis.yaml_to_array_for_select(split_trans[2], :to_s=>1, :selected=>item[col]))]
            else
