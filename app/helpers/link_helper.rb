...
#    ret += "<br />"
  ...
  def current_class(current, name)
    current == name ? 'current' : ''
  end

  def link_to_auth(name, options = {}, html_options = {})
    case options.class.to_s
    when 'Hash'
      controller = (options[:controller] || params[:controller]).to_s.gsub(/^\//, '')
      action = options[:action] || :index
    when 'String'
      method = (html_options[:method] || :get).to_s.upcase
      path = Rails.application.routes.recognize_path(options, :method => method)
      controller = path[:controller]
      action = path[:action]
    end

    if controller.present? || action.present?
      if Core.user.has_auth?(:controller => controller, :action => action)
  ...
  def check_auth_to(table_name, priv_name, uid = Core.user.id, gid = Core.user_group.id)
    system = System::Model::Role.get(1, uid ,"_admin", "admin")
    user = System::Model::Role.get(1, uid ,table_name, priv_name)
    group = System::Model::Role.get(2, gid ,table_name, priv_name)
    ret = user || group || system
    return ret
  end

  def link_to_info(name, options = {}, html_options = {}, settings = {})
    if settings.present? && settings[:disaster_status].present? && settings[:disaster_status] == "going"
      return link_to(name, options, html_options)
    else
      return content_tag(:span, name, :class => 'grayout')
    end
  end

  def inputable?(params)
    Core.user.has_auth?(:controller => params[:controller], :action => :input)
  end
end