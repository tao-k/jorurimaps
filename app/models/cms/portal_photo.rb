# encoding: utf-8
class Cms::PortalPhoto < ActiveRecord::Base
  ...
  def web_state_show
    status_select.each{|a| return a[0] if a[1] == web_state}
  end

  def category_select(options={})
  ...
  def category_show
    category_select.each{|a| return a[0] if a[1] == category}
  end

  def cache_clear
    category_select.each{|a|
      Rails.cache.delete "portal_photo_#{a[1]}"
    }
  end
  ...
  def save_with_file(par_item, mode=:create)
    ...
    self.category = par_item[:category]
    ...
    if mode==:update && file.blank?
      ...
      if self.save
        ...
        directory = "/gis/images/category/#{dir}/"
