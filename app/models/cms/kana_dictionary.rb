...
class Cms::KanaDictionary < ActiveRecord::Base
  ...
  def self.dic_mtime(type)
    ...
    if dic
      ...
      file = "#{Rails.root.to_s}/ext/morph/#{dic}/cmsdic.da"
  ...
  def search_category(str, type)
    unless @sh
      require 'shell'
      @sh = Shell.cd("#{Rails.root.to_s}/ext")
    end
    if type == :ruby
      @sh.cd("#{Rails.root.to_s}/ext")
      chasenrc = './config/chasenrc_ruby'
    else
      @sh.cd("#{Rails.root.to_s}/ext/gtalk")
  ...
  def self.make_dic_file
    ...
    ## ipadic
    dir = "#{Rails.root.to_s}/ext/morph/ipadic"
    ...
    ## unidic
    dir = "#{Rails.root.to_s}/ext/morph/unidic"
    Dir.mkdir(dir) unless Dir.exists?(dir)
