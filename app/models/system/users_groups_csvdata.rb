...
class System::UsersGroupsCsvdata < ActiveRecord::Base
    ...
    @groups.each do |group|
      ...
      group.children.each do |c_group|
        ...
        c_group.users.each do |user|
          ...
          csv_row << user.auth_no
          csv_row << Gis.date_str(user_group.start_at)
          csv_row << Gis.date_str(user_group.end_at)
          csv << csv_row
        end
      end
    end
  ...
  def self.csvup(params)
    ...
    ldap_states = Gis.yaml_to_array_for_select('system_users_ldaps')
    ldap_states_0 = ldap_states.map {|x| x[0]}
    states = Gis.yaml_to_array_for_select('system_states')
    states_0 = states.map {|x| x[0]}
    job_order_states = Gis.yaml_to_array_for_select('system_ugs_job_orders')
    job_order_states_0 = job_order_states.map {|x| x[0]}
    mobile_access_states = Gis.yaml_to_array_for_select('t1f0_kyoka_fuka')
    ...
    if par_item.nil? || par_item[:nkf].nil? || par_item[:file].nil?
      ...
      if extname != '.csv'
        ...
        if file.blank?
          ...
          uniq_groups_code_data = Gis.get_uniq_data(groups_code_data) # 重複したコード
          uniq_users_code_pcode_data = Gis.get_uniq_data(users_code_pcode_data) # 重複したコード
          uniq_users_code_honmu_data = Gis.get_uniq_data(users_code_honmu_data) # 重複したコード
          ...
          csv_hashed.each do |row|
            ...
            if row[:state].blank?
              ...
              row[:error] << "「#{csv_header[:state]}」に、「#{Gis.join(states_0, "・")}」以外のデータが入っています。"
            end
            ...
            elsif Gis.half_width_characters?(row[:parent_code]) != true
            ...
            if row[:ldap].blank?
              ...
              row[:error] << "「#{csv_header[:ldap]}」に、「#{Gis.join(ldap_states_0, "・")}」以外のデータが入っています。"
            end
            ...
            if row[:data_type] == "user"
              if row[:job_order].blank?
                ...
                row[:error] << "「#{csv_header[:job_order]}」に、「#{Gis.join(job_order_states_0, "・")}」以外のデータが入っています。"
              end
            end
            ...
            if row[:data_type] == "user"
              ...
              elsif Gis.half_width_characters?(row[:code]) != true
                row[:error] << "「#{csv_header[:code]}」は、#{hankaku_error_string}"
              end
            end
            ...
            if row[:data_type] == "user"
              if row[:mobile_access].blank?
                ...
                row[:error] << "「#{csv_header[:mobile_access]}」に、「#{Gis.join(mobile_access_states_0, "・")}」以外のデータが入っています。"
              end
            end
            ...
            # 電子メールアドレス
            if row[:email].present? && Gis.is_simplicity_valid_email_address?(row[:email]) != true
    ...
    if error_msg.empty? # エラー無し
      ...
      check[:result] = false
      if error_row_cnt == 0
        check[:error_msg] = Gis.join(error_msg, '<br />')
  ...
  def self.set_autoincrement_number
    ...
    id = nz(id, 0) + 1
    connect = self.connection()
    truncate_query = "ALTER SEQUENCE  #{self.table_name}_id_seq restart with 1;"
    connect.execute(truncate_query)
  end
end