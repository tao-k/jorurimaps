...
class System::User < ActiveRecord::Base
  ...
  def show_group_name(error = Gis.user_groups_error)
  ...
  def self.mobile_access_show(mobile)
    # CSV出力・登録用
    mobile_access = Gis.yaml_to_array_for_select 't1f0_kyoka_fuka'
  ...
  def self.is_dev?(uid = Site.user.id)
    ...
    return false if user.auth_no.to_i != 5
    return true
  end
  ...
  def self.is_editor?(uid = Site.user.id)
    user = System::User.where(:id => uid).first
    return false if user.blank?
    return false if user.auth_no.to_i != 3
    return true
  end
  ...
  def self.get_user_select(g_id=nil,all=nil, options = {})
    ...
    conditions="system_users.state='enabled' and system_users_groups.group_id = #{gid}" if f_ldap.blank?
    conditions="system_users.state='enabled' and system_users_groups.group_id = #{gid} and system_users.ldap = 1" unless f_ldap.blank?
    order = "system_users.sort_no"
    users_select = System::User.find(:all,:conditions=>conditions,:select=>"id,code,name",:order=>order,:joins=>'left outer join system_users_groups on system_users.id = system_users_groups.user_id')
    #users_select = System::User.find(:all,:conditions=>conditions,:select=>"id,code,name",:order=>order,:include=>:groups)
    selects += users_select.map{|user| [ Gis.trim(user.display_name),user.id]}
    return selects
  end
  ...
  def auth_select
    [["一般ユーザ", 2],["承認ユーザ",4],["管理者",5]]
  end

  def auth_show
    select = auth_select
    select.each{|a| return a[0] if a[1] == auth_no}
    return nil
  end
