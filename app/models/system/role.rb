...
class System::Role < ActiveRecord::Base
  ...
  def self.is_admin?(uid = Site.user.id)
    System::Model::Role.get(1, uid ,'role', 'admin')
  end
  ...
  def role_classes
     Gis.yaml_to_array_for_select('system_role_classes')
  end
  ...
  def privs
    Gis.yaml_to_array_for_select('t1f0_kyoka_fuka')
  end
  ...
  def self.td_criteria
    {
      'uid' => Proc.new{|item|
        case item.class_id
        when 1
          "#{get_uname :uid=>item.uid}"
        when 2
          "#{get_gname :gid=>item.uid}"
        else
          ''
        end
      }
  ...
  def uid_label
    label = ''
      case self.class_id
      when 1
        label = get_uname :uid=>self.uid
      when 2
        label = get_gname :gid=>self.uid
      end
    return label
  end
