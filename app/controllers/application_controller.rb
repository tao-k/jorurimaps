...
###  include Cms::Controller::Public
  ...
  after_filter :inline_css_for_mobile, :ruby_view
...
###    Page.error = status
    ...
    if request_uri =~ /\/ud/ && request_uri !~ /_admin/
      path_prefix = "#{Rails.public_path}/ud"
    else
      path_prefix = "#{Rails.public_path}"
    end

    ## Render
    file = "#{path_prefix}/500.html"
...
###    els
    if request.mobile?
      if FileTest.exist?("#{path_prefix}/#{status}_mobile.html")
        file =  "#{path_prefix}/#{status}_mobile.html"
      else
        if FileTest.exist?("#{path_prefix}/#{status}.html")
          file = "#{path_prefix}/#{status}.html"
        end
      end
    else
      if FileTest.exist?("#{path_prefix}/#{status}.html")
