# encoding: utf-8
class System::Admin::RoleNamePrivsController < System::Controller::Admin::Base
  ...
  def create
    ...
    location  = "#{url_for({:action => :index})}?#{@params_set}"
    options = {
  ...
  def update
    ...
    location  = "#{url_for({:action => :show})}?#{@params_set}"
    options = {
      :success_redirect_uri =>  location,
