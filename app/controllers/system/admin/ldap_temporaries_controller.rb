# encoding: utf-8
class System::Admin::LdapTemporariesController < System::Controller::Admin::Base
  ...
  def index
    ...
    @items = item.find(:all, :select=>"DISTINCT ON (version)*")
...
private
  ...
  def _synchro
    ...
    sql += " SET end_at = '#{Time.now.strftime("%Y-%m-%d %H:%M:%S")}' FROM system_users "
    sql += " WHERE system_users.id = system_users_groups.user_id"
    sql += " AND system_users.ldap = 1 AND system_users.ldap_version IS NULL AND system_users_groups.end_at IS NULL AND system_users_groups.job_order = 0"
    sql += " ; "
